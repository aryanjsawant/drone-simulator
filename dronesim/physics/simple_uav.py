...
class SimpleUAVDronePhysics(DronePhysicsEngine):
    ...
    def step(self, action: StepActionType, dt: float = None) -> SimplePhysicsStateType:
        ...
        if op is not None:
            ...
            elif op == DroneAction.LAND:
                logging.info(f"LAND command received. Current operation: {self._state['operation']}")
                if self._state['operation'] == DroneState.IN_AIR:
                    self._state['operation'] = DroneState.LANDING
                    self._state['control'].z.setpoint = 0.0 # Target ground level
                    self._state['_tickLogs'][DroneState.LANDING] = 0
                    logging.info("Drone state set to LANDING, target Z set to 0.0")
        ...
        elif self._state['operation'] == DroneState.LANDING:
            z_error = self._state['control'].z.setpoint - self._state['pos'].z
            logging.info(f"LANDING: Z-pos: {self._state['pos'].z:.2f}, Z-error: {z_error:.2f}, Abs-speed: {glm.length(self._state['pvel'].xyz):.2f}")
            if self._state['pos'].z <= 0.1 and glm.length(self._state['pvel'].xyz) < 0.1: # Close to ground and nearly stopped
                self._state['_tickLogs'][DroneState.LANDING] += 1
                if self._state['_tickLogs'][DroneState.LANDING] > self.TAKEOFF_COMPLETE_STABLE_TICKS: # Reuse for landing stability
                    self._state['operation'] = DroneState.LANDED
                    logging.info("Drone state set to LANDED.")
